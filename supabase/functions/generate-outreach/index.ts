...
serve(async (req) => {
  ...
  try {
    ...
    const signalText = Array.isArray(signals) && signals.length > 0
      ? signals.join(", ")
...
Write a short LinkedIn message or email. 3-4 sentences. End with a soft ask like "open to a quick chat?" or "worth a conversation?"`;
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are a concise recruiting assistant. Write natural, human outreach messages. Never use em dashes." },
